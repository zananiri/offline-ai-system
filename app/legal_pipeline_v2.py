...
#     sources:\n\n" wrapper text
#   = ~5344, rounded down to 5000 for a safety cushion against the weight
#   estimate itself being imperfect. deepseek_analyze's own budget (11264 -
#   2048 - 800 = ~8416) is far more generous, so this same trim safely
#   covers both callers -- see _trim_sources_to_budget below.
_SOURCES_CONTEXT_MAX_WEIGHT = 5000
...
def deepseek_analyze(user_text: str, sources: list[dict], model: str = DEEPSEEK_MODEL_DEFAULT) -> str:
    ...
    return _ollama_chat(model, messages, num_predict=_ANALYZE_NUM_PREDICT, num_ctx=_DEEPSEEK_NUM_CTX)
...
_DRAFT_SYSTEM_PROMPT = (
    ...
    "משפטי שכבר בוצע. שמור במדויק על המשמעות המשפטית ועל כל האזכורים "
    "(שם החוק, מספר הסעיף) מהניתוח -- אל תוסיף מקור משפטי חדש, אל תמציא "
    "אזכור. הפוך את הניתוח לתשובה קריאה וברורה, וציין בבירור כל אי-ודאות "
    "שהניתוח הצביע עליה. סיים תמיד בהערה שזו אינה תחליף לייעוץ מעורך דין "
    "מוסמך."
)


def dicta_draft(user_text: str, analysis: str, history: list[dict] | None = None) -> str:
    messages = [{"role": "system", "content": _DRAFT_SYSTEM_PROMPT}]
    messages += (history or [])
    messages.append({
        "role": "user",
        "content": f"שאלת המשתמש: {user_text}\n\nהניתוח המשפטי שבוצע:\n\n{analysis}",
    })
    return _ollama_chat(DICTA_MODEL, messages, num_predict=_DRAFT_NUM_PREDICT, num_ctx=_DEEPSEEK_NUM_CTX)
...
def deepseek_verify(user_text: str, sources: list[dict], analysis: str, draft: str,
    ...
    try:
        raw = _ollama_chat(model, messages, format_json=True,
                            num_predict=_VERIFY_NUM_PREDICT, num_ctx=_DEEPSEEK_NUM_CTX)
        parsed = _parse_json_response(raw)
    except RuntimeError as e:
        # Verification itself failing (Ollama unreachable, etc.) shouldn't
        # block the answer entirely -- degrade to PASS-with-a-note rather
        # than silently dropping the whole verification step.
        return {"status": "PASS", "issues": [f"(verification step failed to run: {e})"],
                "needs_more_retrieval": False}
    if not parsed or parsed.get("status") not in ("PASS", "FAIL"):
        return {"status": "PASS", "issues": ["(verifier returned an unparseable response)"],
                "needs_more_retrieval": False}
    return {
...
def answer_legal_question(user_text: str, history: list[dict] | None = None,
    ...
    for cycle in range(1, MAX_VERIFICATION_CYCLES + 1):
        ...
        sources = _trim_sources_to_budget(hybrid_retrieve(queries))
        ...
        draft = dicta_draft(combined_question, analysis, history)
