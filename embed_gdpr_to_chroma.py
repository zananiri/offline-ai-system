#!/usr/bin/env python3
"""
Embed & vectorize the GDPR (Regulation (EU) 2016/679) into ChromaDB
---------------------------------------------------------------------
Reads gdpr_articles.jsonl (produced by scrape_gdpr.py), embeds each
article's `embed_text` field using nomic-embed-text served locally by
Ollama, and upserts everything into a persistent ChromaDB collection.

This is the GDPR counterpart of embed_to_chroma.py (Canon AI / CIC IT) --
same model, same batching/retry/dedupe/skip-unchanged approach, same
Ollama endpoints -- just pointed at a different JSONL and collection, so
it can live in the same Chroma store alongside the Canon AI collection
...
scrape_gdpr.py and this project's other one-off scripts):
    python embed_gdpr_to_chroma.py \
        --jsonl ./gdpr_output/gdpr_articles.jsonl \
        --collection gdpr

Defaults --chroma-dir to ./app/chroma_db (i.e. app/chroma_db relative to
wherever you run this from) -- the SAME directory app/ui.py's Canon AI tab
already reads its "cic_it" collection from (see GDPR_CHROMA_DIR /
CANON_CHROMA_DIR in app/ui.py, both of which resolve to app/chroma_db).
This script writes into a separate "gdpr" collection inside that same
directory, so it coexists with "cic_it" without touching it -- override
with --chroma-dir if your chroma_db actually lives somewhere else.
...
- Nomic's model expects a task-instruction PREFIX on every input string
  for best retrieval quality:
    ...
    - "search_query: "     -> prefix to use later on the user's question
                               ...
                               this exact prefix is also what app/ui.py's
                               GDPR AI tab uses at query time)
  Skipping these prefixes still works but measurably hurts retrieval
  quality versus the model's intended usage.
- Articles are embedded in batches via Ollama's /api/embed endpoint
...
def embed_texts_batch(texts: list[str], session: requests.Session, batch_url: str,
    ...
    """
    Embeds multiple articles in one HTTP call via Ollama's /api/embed
...
def content_hash(text: str) -> str:
    ...
    vector's metadata so a later rerun can tell whether an article's content
...
def sanitize_metadata(record: dict, text_hash: str = "") -> dict:
    ...
    lists (hence `recitals` -- a list in the JSONL -- gets joined into a
    comma-separated string here)."""
    meta = {
        "article_number": record.get("article_number") or 0,
        "title": record.get("title") or "",
        "chapter_number": record.get("chapter_number") or 0,
        "chapter_title": record.get("chapter_title") or "",
        "hierarchy_path": record.get("hierarchy_path") or "",
        "source_url": record.get("source_url") or "",
        "recitals": ",".join(str(r) for r in (record.get("recitals") or [])),
    }
...
def dedupe_records(records: list[dict]) -> list[dict]:
    ...
    scrape_gdpr.py run (article numbers are inherently unique), but kept
    for the same reason embed_to_chroma.py has it: Chroma's upsert()
    ...
    dup_count = 0
    for r in records:
        cid = r.get("chunk_id") or f"GDPR-art-{r.get('article_number')}"
...
def main():
    ...
    ap.add_argument("--jsonl", required=True, help="Path to gdpr_articles.jsonl")
    ap.add_argument(
        "--chroma-dir", default="./app/chroma_db",
        help="ChromaDB persistent storage dir (default: app/chroma_db relative to "
             "the current directory -- the same store app/ui.py's Canon AI tab uses, "
             "assuming you're running this from the project root)",
    )
    ap.add_argument("--collection", default="gdpr", help="ChromaDB collection name")
    ...
    ap.add_argument("--batch-size", type=int, default=EMBED_BATCH_SIZE,
                     help="Articles embedded per /api/embed call")
    ap.add_argument("--skip-unchanged", action="store_true",
                     help="On a rerun, skip re-embedding articles whose text hasn't "
    ...
    print(f"Loaded {len(records)} article chunk(s).")
    ...
    for record in records:
        chunk_id = record.get("chunk_id") or f"GDPR-art-{record.get('article_number')}"
    ...
    if skipped_unchanged:
        print(f"--skip-unchanged: {skipped_unchanged} article(s) unchanged since last run, skipping re-embedding.")
    print(f"Embedding {len(pending)} article(s)...")
...
def query_example(question: str, chroma_dir: str = "./app/chroma_db", collection_name: str = "gdpr", n_results: int = 5):
